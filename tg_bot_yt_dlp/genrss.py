...
def getFiles(dirname, extensions=None, recursive=False):
    ...
    if extensions is not None:
        for ext in set([e.lower() for e in extensions]):
            selectedFiles += [
                n for n in allFiles if fnmatch.fnmatch(n.lower(), "*{0}".format(ext))
            ]
    else:
...
def buildItem(
    link, title, guid=None, description="", pubDate=None, indent="   ", extraTags=None
):
    """
    ...
    return "{0}<item>\n{1}{2}{3}{4}{5}{6}{0}</item>".format(
        indent * 2, guid, link, title, descrption, pubDate, extra
    )
...
def fileToItem(host, fname, pubDate, use_metadata=False):
    ...
    if fileMimeType is not None and (
        "audio" in fileMimeType or "video" in fileMimeType or "image" in fileMimeType
    ):
        tagParams = 'url="{0}" type="{1}" length="{2}"'.format(
            fileURL, fileMimeType, os.path.getsize(fname)
        )
...
def gen_zc(dirname, outdir):
    host = "http://zavtra.bublik.com/"

    title = "Zabka pokast"
    description = "Zaba Dushit"
    outfile = "feed.rss"
    extensions = "mp3"
    recursive = False
    sort_creation = True
    ...
    fileNames = getFiles(dirname, extensions=extensions, recursive=recursive)
    if len(fileNames) == 0:
    ...
    sortedFiles = (
        (f[0], time.strftime("%a, %d %b %Y %H:%M:%S +0000", time.gmtime(f[1])))
    ...
    #    print(f"file = {file}")
    # build items
    items = [fileToItem(host, fname, pubDate, False) for fname, pubDate in sortedFiles]
    ...
    if image is not None:
        if image.lower().startswith("http://") or image.lower().startswith("https://"):
            ...
            imgurl = urllib.parse.quote(host + image, ":/")
...
def main(argv=None):
    ...
    try:
        ...
        # build items
        items = [
            fileToItem(host, fname, pubDate, opts.use_metadata)
            for fname, pubDate in sortedFiles
        ]
